...
import { Lang, TranslationKey, translations } from "@/app/(tabs)/translations";
...
export const I18nProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    (async () => {
      try {
        ...
        if (stored === "ar" || stored === "en") {
          ...
          // infer from device locale (basic)
          const sysLocale =
            // Intl might not be available in all RN environments; guard defensively
            typeof Intl !== "undefined"
              ? (Intl.DateTimeFormat().resolvedOptions().locale || "en").split(
                  "-"
                )[0]
              : "en";
  ...
  const t = (key: TranslationKey, vars?: Record<string, string | number>) => {
    ...
    if (fromCurrent == null && fromFallback == null) {
      // in dev, warn about missing key
      if (__DEV__) {
        // eslint-disable-next-line no-console
        console.warn(`Missing translation for key "${key}" in both "${language}" and "en"`);
      }
    }
    const raw = fromCurrent ?? fromFallback ?? String(key);
